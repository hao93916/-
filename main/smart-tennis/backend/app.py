...
def analyze_video(file_id):
    ...
    try:
        ...
        bounces = []
        if tracking_results.get('trajectories'):
            for traj in tracking_results['trajectories']:
                traj_bounces = speed_analyzer.analyze_ball_bounce(traj)
                bounces.extend(traj_bounces)
            ...
            if frontend_status == "完成" and os.path.exists(analysis_file_path):
                try:
                    ...
                    data = {
                        ...
                        "bounces": analysis_data.get('bounces', []) # 🌟 新增這裡：送出落點資料
                    }
                    error = None
                except Exception as e:
                    frontend_status = "失敗" 
