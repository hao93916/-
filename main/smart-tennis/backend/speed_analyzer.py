...
class SpeedAnalyzer:
    ...
    def analyze_ball_bounce(self, trajectory):
        ...
        try:
            ...
            if isinstance(trajectory, dict) and 'positions' in trajectory:
                positions = trajectory['positions']
                frames = trajectory.get('frames', [0] * len(positions)) 
            elif isinstance(trajectory, list) and len(trajectory) > 0 and 'position' in trajectory[0]:
                positions = [pt['position'] for pt in trajectory]
                frames = [pt['frame'] for pt in trajectory]
            else:
                return []

            # 2. 軌跡點太少無法構成 V 字型，直接跳過
            if len(positions) < 3:
                return []
            
            y_coords = [pos[1] for pos in positions]
            
            # 3. 找出這段軌跡的「最低點」 (影像中 Y 越大代表越靠近地面)
            max_y = max(y_coords)
            max_index = y_coords.index(max_y)
            
            # 4. 判斷是否為真實落點
            # 條件 A：最低點不能是這段軌跡的頭或尾 (必須要有落下跟彈起的過程)
            if 0 < max_index < len(y_coords) - 1:
                
            # 條件 B：計算落下與彈起的高低差
                drop_diff = max_y - y_coords[0]
                rise_diff = max_y - y_coords[-1]
                
                # 🌟 降低門檻：改成只要大於 1 像素就算數！
                if drop_diff > 1 and rise_diff > 1:
                    print(f"🎾 [Debug] 成功抓到落點！ Frame: {frames[max_index]}, 高度: {max_y}") # 加這行看終端機
                    bounces.append({
